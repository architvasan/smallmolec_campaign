...
def train_val_data(hyper_params):
    ...
    if hyper_params['data_loading']['stratified']:
        ...
        data = data[data['scores']<5]
        data['scores'] = [-1*d for d in data['scores']]#data['DockingScore'].apply(lambda x: -1*x+ 5)#np.array(-1* data['DockingScore'])+5
        ...
        #print(data.describe())
        y = data['scores']
        bin_left = 0#int(min(y))+1
        bin_right = 14#int(max(y))
    ...
    y_train = data_train["Scores"][1:].values.reshape(-1, 1) * 1.0 
    y_val = data_vali["Scores"][1:].values.reshape(-1, 1) * 1.0
