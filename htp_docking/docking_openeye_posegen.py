...
def run_docking(
    ...
    if best_score[0]<score_cutoff:
        ...
        os.makedirs(f'{out_lig_dir}/pdbs', exist_ok=True)
        os.makedirs(f'{out_lig_dir}/dcds', exist_ok=True)
...
def run_mpi_docking(
    ...
    if not pose_gen:
        for batch_it, (batch_s, batch_i) in tqdm(enumerate(zip(smiles_list_batches, smiles_index_batches))):
            scores = [run_docking_score(smi, f'{smi_ind}', receptor, max_confs, temp_dir) for smi, smi_ind in zip(batch_s, batch_i )]
            df_new = pd.DataFrame({'SMILES':batch_s,'Scores':scores})
            df_new.to_csv(f'{out_csv_pattern}.{batch_it}.{rank}.csv', index=False)
...
def run_mpi_docking_list(
    ...
    #smiles_list_new = sample_dataset(data_pwd, dataset, size, rank, sample_size)
    smiles_list_new = pd.read_csv(data_smi)#, sep='\t', names=['smiles', 'name'])

    smiles_list_new = np.array_split(smiles_list_new['smiles'], size)[rank]
    # Read the receptor to dock to
    receptor = read_receptor(receptor_oedu_file)
    smiles_index_new = [ind for ind in range(len(smiles_list_new))]
    smiles_list_batches = np.array_split(smiles_list_new, int(len(smiles_list_new)//batch_size))
    ...
    out_lig_dir = f'{out_lig_dir}/{rank}'
    temp_dir = f'{temp_dir}/{rank}'
...
if __name__ =="__main__":
    ...
    parser.add_argument("-r", "--rec", type=Path)
    parser.add_argument('-m', '--usempi', type=str, default='no', required=False, help='yes/no to use mpi')
    ...
    data_list = config['smiles_input']
    
    recep_file = f'{config["recep_loc"]}/{args.rec}' # receptor oedu file for openeye 
    ...
    score_dir = config['score_directory']
    os.makedirs(score_dir, exist_ok=True)
    
    score_pattern_base = config['score_pattern']#'scores/4ui5_scores' #store scores like this (will have #ranks files)
    score_pattern = f'{score_dir}/{score_pattern_base}'
    protein_pdb = config['protein_pdb'] #protein pdb file to use to store. Will save everything in this file to complex
    sample_size = 0#config['sample_size'] 
